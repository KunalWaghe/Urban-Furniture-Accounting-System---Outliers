...
| **P0** | Purchase Orders List | `/purchase-orders` | View list of POs, filter by status, server-side sort and pagination, quick action to create | loading / empty / error / success | Implemented with live API (search, status filter, sortable PO#/date/total, pagination, summary cards) |
| **P0** | Purchase Order Detail | `/purchase-orders/[id]` | Track PO status, confirm/cancel drafts, edit drafts, convert to Bill (disabled placeholder), inspect lines and budget analytics | loading / mutating / error / success / not-found | Implemented with live API; Create Bill intentionally disabled until the Vendor Bill workflow lands |
| **P0** | Purchase Order New/Edit | `/purchase-orders/new`, `/purchase-orders/[id]/edit` | Create or edit draft POs with vendor search, line items, live totals, budget-exceeded warning | loading / validating / submitting / error / success | Implemented with live API; edit is draft-only (confirmed/cancelled redirect to detail) |
| **P0** | Sales Orders List | `/sales-orders` | View list of SOs, filter by status, client-side sort and pagination over adapter data, quick action to create | loading / empty / error / success | Implemented with demo adapter; live API wiring queued (no sales-order router yet) |
