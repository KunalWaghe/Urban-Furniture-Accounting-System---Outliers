...
export function ProductsPage() {
  ...
  const productsQuery = useQuery({ queryKey: ["products", "master-data"], queryFn: fetchProducts });
  ...
  const filteredProducts = useMemo(() => products.filter((product) => { const q = search.trim().toLowerCase(); return (categoryFilter === "all" || product.category === categoryFilter) && (!q || [product.name, product.category, product.product_type, product.description].some((value) => value?.toLowerCase().includes(q))); }), [products, categoryFilter, search]);

  const saveMutation = useMutation({ mutationFn: () => editing ? updateProduct(editing.id, form) : createProduct(form), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["products"] }); setEditing(null); setForm(emptyForm); setIsModalOpen(false); setError(null); }, onError: (err) => setError(err instanceof Error ? err.message : "Could not save product.") });
  const deleteMutation = useMutation({ mutationFn: (id: number) => deleteProduct(id), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["products"] }); setDeletingProduct(null); } });
  function openCreate() { setEditing(null); setForm(emptyForm); setError(null); setIsModalOpen(true); }
  function openEdit(product: Product) { setEditing(product); setForm({ name: product.name, product_type: product.product_type, category: product.category ?? "", price: product.price, cost: product.cost, tax_percent: product.tax_percent, description: product.description ?? "" }); setError(null); setIsModalOpen(true); }
  function updateField(field: keyof ProductInput, value: string | number | null) { setForm((current) => ({ ...current, [field]: value })); }
  function submitForm(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); if (!form.name.trim()) { setError("Name is required."); return; } if (form.price < 0 || !Number.isFinite(form.price)) { setError("Sales price must be zero or more."); return; } saveMutation.mutate(); }

  const columns: DataTableColumn<Product>[] = [
    { key: "name", label: "Product", render: (product) => <div><p className="font-semibold text-text">{product.name}</p><p className="mt-0.5 text-xs text-text-muted">{product.description || "No description"}</p></div> },
    { key: "product_type", label: "Type", render: (product) => <Badge variant="secondary">{productTypeLabel(product.product_type)}</Badge> },
    { key: "category", label: "Category", render: (product) => <span className="text-text-muted">{product.category || "Uncategorized"}</span> },
    { key: "price", label: "Sales price", render: (product) => <span className="font-semibold text-text">{formatINR(product.price)}</span> },
    { key: "cost", label: "Cost price", render: (product) => <span className="text-text-muted">{product.cost == null ? "—" : formatINR(product.cost)}</span> },
    {
      key: "actions", label: "", render: (product) => (
        <div className="flex items-center gap-1">
          <Button type="button" variant="ghost" size="sm" onClick={() => openEdit(product)}><Edit3 className="h-3.5 w-3.5" />Edit</Button>
          {product.is_active && (
            <Button type="button" variant="ghost" size="sm" onClick={() => setDeletingProduct(product)} className="text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950/30">
  ...
  return <div className="space-y-6 pb-12">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary-600">Account / Master data</p><h1 className="mt-2 text-2xl font-bold tracking-tight text-text sm:text-3xl">Products</h1><p className="mt-1 max-w-2xl text-sm text-text-muted">Manage your furniture catalogue, pricing, tax, and product categories.</p></div><Button type="button" onClick={openCreate} className="w-full sm:w-auto"><Plus className="h-4 w-4" />New product</Button></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Card size="sm"><CardContent className="flex items-center gap-3"><div className="rounded-lg bg-primary-50 p-2.5 text-primary-600 dark:bg-primary-950/40"><Package className="h-5 w-5" /></div><div><p className="text-xs text-text-muted">Catalogue items</p><p className="text-xl font-bold text-text">{products.length}</p></div></CardContent></Card><Card size="sm"><CardContent><p className="text-xs text-text-muted">Categories</p><p className="mt-1 text-xl font-bold text-text">{categories.length}</p></CardContent></Card><Card size="sm"><CardContent><p className="text-xs text-text-muted">Goods / Services</p><p className="mt-1 text-xl font-bold text-text">{products.filter((product) => product.product_type === "goods").length} / {products.filter((product) => product.product_type === "service").length}</p></CardContent></Card></div>
    <Card><CardContent className="p-4 sm:p-5"><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold text-text">Product catalogue</h2><p className="mt-1 text-xs text-text-muted">Switch views to scan prices quickly or browse products by category.</p></div><div className="flex flex-col gap-2 sm:flex-row"><div className="relative"><Tag className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-text-muted" /><select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="w-full rounded-lg border border-border bg-surface py-2 pl-9 pr-8 text-xs text-text outline-none focus:border-primary-500 sm:w-auto" aria-label="Filter products by category"><option value="all">All categories</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></div><div className="flex rounded-lg border border-border bg-surface-muted p-1"><button type="button" onClick={() => setView("table")} className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium ${view === "table" ? "bg-surface text-primary-600 shadow-sm" : "text-text-muted"}`}><List className="h-3.5 w-3.5" />Table</button><button type="button" onClick={() => setView("kanban")} className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium ${view === "kanban" ? "bg-surface text-primary-600 shadow-sm" : "text-text-muted"}`}><Grid2X2 className="h-3.5 w-3.5" />Kanban</button></div></div></div>
      {view === "table" ? <DataTable columns={columns} data={filteredProducts} loading={productsQuery.isLoading} onSearch={setSearch} searchPlaceholder="Search products..." emptyTitle="No products found" emptyDescription="Add a product or adjust your filters." /> : <ProductKanban products={filteredProducts} onEdit={openEdit} onDelete={(p) => setDeletingProduct(p)} search={search} onSearch={setSearch} loading={productsQuery.isLoading} />}
    </CardContent></Card>
