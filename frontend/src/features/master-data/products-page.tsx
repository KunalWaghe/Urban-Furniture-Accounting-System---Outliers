...
function ProductKanban({ products, onEdit, onDelete, search, onSearch, loading }: { products: Product[]; onEdit: (product: Product) => void; onDelete: (product: Product) => void; search: string; onSearch: (value: string) => void; loading: boolean }) {
  ...
  return <div className="space-y-4"><input type="search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search products..." className="w-full max-w-sm rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20" />{groups.length === 0 ? <div className="rounded-xl border border-dashed border-border px-6 py-12 text-center text-sm text-text-muted">No products found.</div> : <div className="grid gap-4 overflow-x-auto xl:grid-cols-3">{groups.map((group) => { const groupProducts = products.filter((product) => (product.category || "Uncategorized") === group); return <section key={group} className="min-h-48 min-w-[260px] rounded-xl bg-surface-muted/60 p-3"><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold text-text">{group}</h3><span className="rounded-full bg-surface px-2 py-0.5 text-xs text-text-muted">{groupProducts.length}</span></div><div className="space-y-3">{groupProducts.map((product) => <div key={product.id} className="group relative w-full rounded-xl border border-border bg-surface p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary-300 hover:shadow-md"><button type="button" onClick={() => onEdit(product)} className="block w-full text-left"><div role="img" aria-label={`${product.name} product image`} className="mb-3 flex h-32 w-full items-center justify-center overflow-hidden rounded-lg bg-primary-50 bg-cover bg-center text-primary-600 dark:bg-primary-950/40" style={product.image_url ? { backgroundImage: `url(${product.image_url})` } : undefined}>{product.image_url ? null : <span className="text-2xl font-bold">{productInitials(product.name)}</span>}</div><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-text">{product.name}</p><Badge variant="secondary" className="mt-2">{productTypeLabel(product.product_type)}</Badge></div></div><div className="mt-4 grid grid-cols-2 gap-3"><div><p className="text-[11px] uppercase tracking-wide text-text-muted">Sales price</p><p className="mt-0.5 text-lg font-bold text-primary-600">{formatINR(product.price)}</p></div><div><p className="text-[11px] uppercase tracking-wide text-text-muted">Cost</p><p className="mt-0.5 text-sm font-semibold text-text">{product.cost == null ? "—" : formatINR(product.cost)}</p></div></div></button><div className="absolute right-3 top-3 flex items-center gap-1 rounded-md bg-surface/90 p-1 shadow-sm"><button type="button" onClick={() => onEdit(product)} className="p-1 text-text-muted hover:text-text" title="Edit product"><Edit3 className="h-3.5 w-3.5" /></button>{product.is_active && <button type="button" onClick={() => onDelete(product)} className="p-1 text-red-600 hover:text-red-700" title="Deactivate product"><Trash2 className="h-3.5 w-3.5" /></button>}</div></div>)}</div></section>; })}</div>}</div>;
}
