...
export function ContactsPage() {
  ...
  const contactsQuery = useQuery({ queryKey: ["contacts", "master-data"], queryFn: fetchContacts });
  ...
  const contacts = useMemo(() => {
    ...
    return typeFilter === "all" ? data : data.filter((contact) => contact.type === typeFilter);
  }, [contactsQuery.data, typeFilter]);
  ...
  function openEdit(contact: Contact) {
    setEditing(contact);
    setForm({ name: contact.name, type: contact.type, email: contact.email ?? "", mobile: contact.mobile ?? "", city: contact.city ?? "", state: contact.state ?? "", pincode: contact.pincode ?? "" });
    setError(null);
    setIsModalOpen(true);
  }
  ...
  const columns: DataTableColumn<Contact>[] = [
    { key: "name", label: "Contact", render: (contact) => <div><p className="font-semibold text-text">{contact.name}</p><p className="mt-0.5 text-xs text-text-muted">#{contact.id}</p></div> },
    { key: "type", label: "Type", render: (contact) => typeBadge(contact.type) },
    { key: "email", label: "Contact details", render: (contact) => <div className="space-y-1 text-xs text-text-muted">{contact.email && <div className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" />{contact.email}</div>}{contact.mobile && <div className="flex items-center gap-1.5"><Phone className="h-3.5 w-3.5" />{contact.mobile}</div>}{!contact.email && !contact.mobile && "No details added"}</div> },
    { key: "city", label: "Location", render: (contact) => <span className="flex items-center gap-1.5 text-sm text-text-muted"><MapPin className="h-3.5 w-3.5 shrink-0" />{[contact.city, contact.state].filter(Boolean).join(", ") || "—"}</span> },
    { key: "is_active", label: "Status", render: (contact) => <Badge variant={contact.is_active ? "default" : "secondary"}>{contact.is_active ? "Active" : "Inactive"}</Badge> },
    { key: "actions", label: "", render: (contact) => (
      <div className="flex items-center gap-1">
        <Button type="button" variant="ghost" size="sm" onClick={() => openEdit(contact)}><Edit3 className="h-3.5 w-3.5" />Edit</Button>
        {contact.is_active && (
          <Button type="button" variant="ghost" size="sm" onClick={() => setDeletingContact(contact)} className="text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950/30">
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary-600">Account / Master data</p><h1 className="mt-2 text-2xl font-bold tracking-tight text-text">Contacts</h1><p className="mt-1 max-w-2xl text-sm text-text-muted">Keep customers and vendors ready to use across every order, bill, and invoice.</p></div>
        <Button type="button" onClick={openCreate}><Plus className="h-4 w-4" />New contact</Button>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <Card size="sm"><CardContent className="flex items-center gap-3"><div className="rounded-lg bg-primary-50 p-2.5 text-primary-600 dark:bg-primary-950/40"><Users className="h-5 w-5" /></div><div><p className="text-xs text-text-muted">Total contacts</p><p className="text-xl font-bold text-text">{contactsQuery.data?.length ?? 0}</p></div></CardContent></Card>
        <Card size="sm"><CardContent><p className="text-xs text-text-muted">Customers</p><p className="mt-1 text-xl font-bold text-text">{(contactsQuery.data ?? []).filter((c) => c.type === "customer" || c.type === "both").length}</p></CardContent></Card>
        <Card size="sm"><CardContent><p className="text-xs text-text-muted">Vendors</p><p className="mt-1 text-xl font-bold text-text">{(contactsQuery.data ?? []).filter((c) => c.type === "vendor" || c.type === "both").length}</p></CardContent></Card>
      </div>

      <Card><CardContent className="p-5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold text-text">Contact directory</h2><p className="mt-1 text-xs text-text-muted">Search by name, type, email, or location.</p></div><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)} className="rounded-lg border border-border bg-surface px-3 py-2 text-xs text-text outline-none focus:border-primary-500" aria-label="Filter contacts by type"><option value="all">All contact types</option><option value="customer">Customers</option><option value="vendor">Vendors</option><option value="both">Customer & Vendor</option></select></div>
        <DataTable columns={columns} data={contacts} loading={contactsQuery.isLoading} searchPlaceholder="Search contacts..." emptyTitle="No contacts found" emptyDescription="Add a contact or adjust the type filter." />
      </CardContent></Card>
