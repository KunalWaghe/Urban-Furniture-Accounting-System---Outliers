...
from app.schemas.auth import AdminUserCreateRequest, AdminUserUpdateRequest, UserProfileResponse
...
def update_user(
    ...
    """
    Update an existing user account (role, profile, password, active status).
    
    Security: Only accessible to users with role 'admin'.
    """
    return auth_service.admin_update_user(db, user_id, req, admin_user)


@router.delete("/{user_id}", response_model=UserProfileResponse, status_code=status.HTTP_200_OK)
def deactivate_user(
    user_id: int,
    db: Session = Depends(get_db),
    admin_user: User = Depends(require_roles(["admin"])),
    ...
    Security: Only accessible to users with role 'admin'.
    """
    return auth_service.admin_deactivate_user(db, user_id, admin_user)
